...
// DB typ
// encapsulates db logic for external use
// all external types are saved as internal types to prevent db package usage outside this package
type DB struct {
...
func (db *DB) FindById(ctx context.Context, id primitive.ObjectID, coll string) *mongo.SingleResult {
	return db.getCollection(coll).FindOne(ctx, bson.M{"_id": id})
}

func (db *DB) FindOneBy(ctx context.Context, filter bson.M, coll string) *mongo.SingleResult {
	return db.getCollection(coll).FindOne(ctx, filter)
}
...
func (db *DB) DeleteBy(ctx context.Context, filter bson.M, coll string) (*DeleteResult, error) {
	return db.getCollection(coll).DeleteOne(ctx, filter)
}
